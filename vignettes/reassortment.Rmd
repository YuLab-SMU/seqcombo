---
title: "seqcombo for genetic reassortment"
...
`plot_reassort` function for producing publication quality figure of
reassortment events.
...
The `plot_reassort` requires two tibble data frame of virus information and
...
```{r fig.width=8, fig.height=7}
plot_reassort(virus_info, flow_info, v_color='firebrick', v_fill='darkgreen', l_color='steelblue')
```
...
plot_reassort(virus_info, flow_info, v_color=~Host, v_fill=~Host)
```
...
The output of `plot_reassort` is a `ggplot` object and users can use `ggplot2`
...
Top-down style is also supported.
...
plot_reassort(virus_info, flow_info, v_color=~Host, v_fill=~Host) + scale_y_reverse() + scale_x_continuous(limits=c(0, 5.5))
```
