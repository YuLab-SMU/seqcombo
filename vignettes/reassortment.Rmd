...
`hybridplot` function for producing publication quality figure of
reassortment events.
...
The `hybridplot` requires two tibble data frame of virus information and
...
```{r fig.width=8, fig.height=7}
hybridplot(virus_info, flow_info, v_color='firebrick', v_fill='darkgreen', l_color='steelblue')
```
...
hybridplot(virus_info, flow_info, v_color=~Host, v_fill=~Host)
```
...
hybridplot(virus_info, flow_info, v_color=~Host, v_fill=~Host, asp=2)
```



The output of `hybridplot` is a `ggplot` object and users can use `ggplot2`
...
hybridplot(virus_info, flow_info, parse='emoji', t_size=8, t_color='firebrick')
