...
If your data already include sampling time, a convenient pattern is to keep the
time in a table column and let `layout_timeline()` arrange the other axis. The
example below starts from a segment-level table with one row per
virus-segment combination:
...
timeline_flow <- data.frame(
    from = c("avian_1990", "human_1990", "swine_2000"),
    to = c("swine_2000", "swine_2000", "human_2009"),
    stringsAsFactors = FALSE
)

virus_from_table <- build_virus_info_from_long(
    timeline_segments,
    x = "sample_time",
    keep = "host"
)
virus_from_table <- layout_timeline(virus_from_table, timeline_flow, time_col = "x")
